    ...
    export default {
        ...
        computed: {
            ...
            cells () {
                ...
                const lastDayInMaxDate = this.getLastDaysInMonth(maxDate.getFullYear(),(Number(maxDate.getMonth())+1));
                
                const lastMaxDate = new Date(maxDate.getFullYear(),maxDate.getMonth(),lastDayInMaxDate);
                for (let i = 0; i < 12; i++) {
                    ...
                    // if(this.begin){
                        if(this.type==='range'){
                            //选择时间段的开始时间
                            if(date.getFullYear()===minDate.getFullYear()&&i===Number(minDate.getMonth())&&1<=Number(minDate.getDate())){
                                cell.selected = true;
                                cell.start = true;
                            }
                            //选择时间段的结束时间
                            if(date.getFullYear()===maxDate.getFullYear()&&i===Number(maxDate.getMonth())&&lastDayInMaxDate>=Number(maxDate.getDate())){
                                cell.selected = true;
                                cell.end = true;
                            }
                            //选择时间段的开始时间到结束
                            if(date.getFullYear()>=minDate.getFullYear()&&date.getFullYear()<=maxDate.getFullYear()){
                                if(minDate.getFullYear()===maxDate.getFullYear()&&i>=Number(minDate.getMonth())&&i<=Number(maxDate.getMonth())){
                                    cell.range = true;
                                }else if(minDate.getFullYear()!=maxDate.getFullYear()){
                                    cell.range = ((date.getFullYear()===minDate.getFullYear()&&i>=Number(minDate.getMonth()))||(date.getFullYear()===maxDate.getFullYear()&&i<=Number(maxDate.getMonth())))
                                }
                            }
                        }else{
            ...
            handleClick(cell){
                ...
                if(this.$parent.selectionMode==='month'){
                    ...
                    if(this.handleDateType==='start'){
                        ...
                        if (newDate >= this.minDate) {//顺选
                            newDate.setMonth(month+1);
                            let minDate = new Date(this.minDate.getTime());
                            let maxDate = new Date(newDate.setDate(0));
                            let isOutRange = false;
                            if(!!this.maxRangeDay&&(getDateRange(minDate,maxDate)>366)){
                                maxDate.setTime(minDate.getTime() + 3600 * 1000 * 24 * 365);
                                maxDate.setDate(0);
                                isOutRange = true;
                            }
                            if(this.min&&this.min.getTime()>minDate.getTime()){
                            ...
                            let isOutRange = false;
                            if(!!this.maxRangeDay&&(getDateRange(minDate,maxDate)>366)){
                                minDate.setTime(maxDate.getTime() - 3600 * 1000 * 24 * 365);
                                minDate.setMonth(Number(minDate.getMonth())+1);
                                minDate.setDate(1);
                                isOutRange = true;
                            }
